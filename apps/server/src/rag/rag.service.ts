...
export class RagService implements OnModuleInit {
  ...
  private async startWorker() {
    ...
    this.worker.on('failed', (job, err) => console.error(`[RAG] Failed: ${job?.id}`, err.message))
  ...
  async retrieve(query: string, topK = 5): Promise<Array<{ content: string; documentId: string; similarity: number }>> {
    // Step 1: 向量检索 (召回更多候选)
    ...
    `) as unknown as Array<{ content: string; documentId: string; similarity: number }>

    if (candidates.length === 0) return []

    // Step 2: Reranker 重排序 (显著提升精度)
    if (RERANKER_ENABLED && candidates.length > 1) {
      console.log(`[RAG] Reranking ${candidates.length} candidates with ${RERANKER_MODEL}`)
      const rerankResults = await rerank(query, candidates.map(c => c.content), topK)
  ...
  async getDocumentTitle(documentId: string): Promise<string> {
    const result = await db.select({ title: documents.title })
      .from(documents)
  ...
  async getOutdatedDocuments(): Promise<string[]> {
    const results = await db.selectDistinct({ documentId: documentChunks.documentId })
      .from(documentChunks)
      .where(ne(documentChunks.embeddingModel, EMBEDDING_MODEL))
      .limit(100)
