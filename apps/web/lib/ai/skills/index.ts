...
export const createFlashcards = tool({
  description: '从用户提供的内容创建闪卡。当用户要求"做成闪卡"、"创建卡片"、"帮我记忆"时调用。',
  ...
  execute: async ({ cards, context }) => {
    ...
    for (const card of cards) {
      const flashcard = await flashcardStore.createCard(card.front, card.back, { context })
      created.push({
        id: flashcard.id,
...
export const searchNotes = tool({
  description: '在用户的笔记/文档库中搜索相关内容。当用户问"我之前写过什么关于..."、"搜索笔记"时调用。',
  inputSchema: z.object({
  ...
  execute: async ({ query, limit }) => {
    try {
      ...
      if (!response.ok) {
        return { success: false, query, results: [], message: '搜索服务暂不可用' }
      }
