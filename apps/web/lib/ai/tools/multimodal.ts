...
export const generateIllustrationTool = tool({
  description: "根据描述生成一张高质量的插图，用于增强课程内容。返回图片的 URL。请在 Markdown 中以 ![alt](url) 格式插入。",
  ...
  execute: async ({ prompt, aspectRatio }) => {
    ...
    try {
      ...
      const apiKey = (typeof window === "undefined" ? env.AI_302_API_KEY : undefined) || clientEnv.AI_302_API_KEY;
