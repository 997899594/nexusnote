...
import { sanitizeHtml, smartConvert } from "./markdown";
...
export function getDocumentSummary(structure: DocumentStructure): string {
  ...
  structure.blocks.forEach((block, _i) => {
...
export function applyEditCommand(
  ...
  switch (command.action) {
    ...
    case "insert_after": {
      ...
      editor.chain().focus().setTextSelection({ from: to, to: to }).insertContent(afterHtml).run();
      return true;
    }
    ...
    case "insert_before": {
      ...
      editor.chain().focus().setTextSelection({ from, to: from }).insertContent(beforeHtml).run();
      return true;
    }
