...
function nextInterval(stability: number): number {
  const interval = (stability / 0.9) * (Math.pow(FSRS_PARAMS.requestRetention, 1 / -0.5) - 1)
...
export class FlashcardStore {
   ...
   * 更新卡片内容
   */
  async updateCard(id: string, updates: Partial<Pick<LocalFlashcard, 'front' | 'back' | 'context' | 'tags'>>): Promise<void> {
  ...
  async reviewCard(
    ...
    const now = Date.now()
    const elapsedDays = card.state === State.New
      ? 0
      : Math.max(0, (now - card.due) / (1000 * 60 * 60 * 24))
    ...
    console.log(`[FlashcardStore] Reviewed card ${card.id}: rating=${rating}, next due in ${card.scheduledDays} days`)
    return { card, log }
  }
  ...
  async getStats(): Promise<ReviewStats> {
    ...
    return {
      ...
      learningCards: activeCards.filter(c => c.state === State.Learning || c.state === State.Relearning).length,
