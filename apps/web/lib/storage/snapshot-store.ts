...
import { localDb, STORES, DocumentSnapshot, LocalDocument } from './local-db'
...
export class SnapshotStore {
  ...
  async compareSnapshots(
    ...
    const [before, after] = snap1.timestamp < snap2.timestamp
      ? [snap1, snap2]
      : [snap2, snap1]
   ...
   * 与当前文档对比
   */
  async compareWithCurrent(snapshotId: string, currentYDoc: Y.Doc): Promise<{
    before: string
    after: string
