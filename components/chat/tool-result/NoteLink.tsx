...
export function NoteLink({ output, type }: NoteLinkProps) {
  if (!output.success) {
    return (
      <div className="mt-2 rounded-2xl bg-[var(--color-panel-soft)] p-3">
        <p className="text-sm text-[var(--color-text-secondary)]">{output.error}</p>
      </div>
    );
  }
  ...
  return (
    <div className="mt-2 rounded-2xl bg-[var(--color-panel-soft)] p-3">
      ...
      <a
        href={href}
        className="mt-2 inline-flex items-center gap-1 text-sm text-[var(--color-text-secondary)] hover:text-[var(--color-text)]"
      >
