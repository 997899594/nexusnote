            ...
            if (item.isFloating) {
              return (
                <div key="floating" className="flex justify-center h-full relative">
                    ...
                    <motion.button
                      ...
                      animate={{
                        boxShadow: drawerOpen
                          ? "0 0 0 8px var(--color-accent-ring)"
                          : createOpen
                            ? "0 0 0 8px var(--color-accent-ring)"
                            : "0 10px 25px -5px rgba(0, 0, 0, 0.3)",
                      }}
                      transition={{ duration: 0.2 }}
                      className="flex items-center justify-center w-14 h-14 rounded-full bg-[var(--color-accent)] text-[var(--color-accent-fg)] shadow-lg touch-target"
                      aria-label="菜单"
                    >
                      <motion.div
                  ...
                  <span className="self-end pb-2 text-[10px] font-medium text-[var(--color-text-tertiary)]">
                    {item.label}
                  </span>
                </div>
              );
        ...
        {createOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed bottom-24 left-4 right-4 z-50 bg-[var(--color-surface)] rounded-2xl shadow-2xl p-2 md:hidden"
            >
              {createOptions.map((option, index) => (
                ...
                >
                  <Link
                    href={option.href}
                    onClick={() => setCreateOpen(false)}
                    className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-[var(--color-hover)] transition-colors"
                  >
                    <div className="w-10 h-10 rounded-full bg-[var(--color-hover)] flex items-center justify-center">
                      <option.icon className="w-5 h-5 text-[var(--color-text-secondary)]" />
                    </div>
