...
**现象**：浏览器访问 `http://app.example.com` 时，始终被重定向到 `https://app.example.com`。
**本质原因**：

1.  应用（Next.js）配置了 `NEXT_PUBLIC_APP_URL: "https://app.example.com"`。
2.  当流量通过 80 端口进入时，应用发现访问协议（http）与配置协议（https）不符，主动下发 301 指令。
3.  由于我们当时没有配置 443 端口和证书，浏览器跳转后无法建立连接。

### 3.2 解决方案

我们将 `app.yaml` 中的配置修正为 `http://app.example.com`，并确保网关层正确透传了协议头。通过这种方式，应用意识到了它正处于合法的 HTTP 运行状态，从而停止了重定向行为。
...
**修复**: 给 HTTP listener 显式添加 `hostname: app.example.com`。
...
- `https://app.example.com/` → 200 (Let's Encrypt TLS, 有效期至 2026-05-14)
- `http://app.example.com/` → 301 → HTTPS
