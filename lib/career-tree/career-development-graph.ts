...
import { getCurrentCareerTree, getTreeByDirectionKey } from "@/lib/career-tree/view-model";
...
function candidateTreesAsFutureRoles(
  snapshot: CareerTreeSnapshot,
  currentTree: CandidateCareerTree,
): CareerRoleNode[] {
  return snapshot.trees
    .filter((tree) => tree.directionKey !== currentTree.directionKey)
    .slice(0, 4)
    .map((tree) => ({
      ...toCareerRoleNode(tree, snapshot),
      horizon: "later" as const,
      source: "candidate_tree" as const,
      isSelected: false,
    }));
}
...
export function buildCareerDevelopmentGraph(
  ...
  const currentTree =
    getTreeByDirectionKey(snapshot, directionKey) ?? getCurrentCareerTree(snapshot);
