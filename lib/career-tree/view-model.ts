...
export interface ResolvedCareerTreeDisplayState {
...
export function getCurrentCareerTree(snapshot: CareerTreeSnapshot): CandidateCareerTree | null {
  return (
...
export function resolveCareerTreeDisplayState(params: {
  snapshot: CareerTreeSnapshot;
  directionKey?: string | null;
  focusSnapshot: FocusSnapshotProjection | null;
  profileSnapshot: ProfileSnapshotProjection | null;
}): ResolvedCareerTreeDisplayState | null {
  const currentTree =
    getTreeByDirectionKey(params.snapshot, params.directionKey) ??
    getCurrentCareerTree(params.snapshot);
