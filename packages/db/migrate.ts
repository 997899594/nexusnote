...
async function runMigrations() {
  ...
  try {
    ...
    if (majorVersion > 0 && majorVersion < 13) {
      ...
      console.error("");
      throw new Error(
        `PostgreSQL ${majorVersion} does not support custom extensions (requires 13+)`,
      );
    }
    ...
    `;
    console.log(
      "[Migrate] Currently installed extensions:",
      installedExts.map((e) => `${e.extname}@${e.extversion}`).join(", "),
    );
