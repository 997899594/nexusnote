...
export const DEFAULT_PARAMETERS = {
  w: [
    0.4072, 1.1829, 3.1262, 15.4722, // 初始稳定性 S0 (Again, Hard, Good, Easy)
    7.2102,  // D0: 初始难度
    0.5715,  // 难度衰减
    1.0,     // 难度增益
    0.0062,  // 成功复习难度变化
...
export class FSRS {
  ...
  private handleReview(card: Card, rating: Rating, elapsedDays: number): void {
    ...
    if (rating === Rating.Again) {
      ...
      card.reps += 1
      card.difficulty = this.nextDifficulty(card.difficulty, rating)
      card.stability = this.nextRecallStability(card.difficulty, card.stability, retrievability, rating)
      card.scheduledDays = this.nextInterval(card.stability)
    }
  }
  ...
  private nextInterval(stability: number): number {
    const interval = (stability / 0.9) * (Math.pow(this.requestRetention, 1 / -0.5) - 1)
